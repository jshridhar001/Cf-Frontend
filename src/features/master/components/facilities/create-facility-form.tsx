...
interface CreateFacilityFormProps {
  onSuccess?: (values: CreateFacilityFormValues) => void;
  onCancel?: () => void;
}

export function CreateFacilityForm({ onSuccess, onCancel }: CreateFacilityFormProps) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const submitted: CreateFacilityFormValues = {
        name: value.name.trim(),
        usedIn: value.usedIn,
      };

      await createFacility(submitted);
      form.reset();
      onSuccess?.(submitted);
    },
  });
