...
import { type GenerationsTableMeta, type GenerationsTableRow } from './generation-table-column';

interface GenerationsTableProps<TData extends RowData> {
...
export function GenerationsTable<TData extends RowData>({
  columns,
  data,
  onAdd,
  onDeleteAll,
  ...
  const table = useTable({
    ...
    meta: {
      onEdit,
      onDelete,
    } satisfies GenerationsTableMeta,
    state: {
      sorting,
      columnFilters,
    },
  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-4">
      ...
      <ItemGroup className="md:hidden">
        {table.getRowModel().rows.length ? (
          table.getRowModel().rows.map((row) => {
            const generation = row.original as GenerationsTableRow;
            return (
              <Item key={row.id} variant="outline" size="sm" className="items-start">
                <ItemHeader className="gap-3">
                  <ItemContent className="min-w-0 pr-1">
                    <ItemTitle>{generation.name}</ItemTitle>
                    <ItemDescription>{formatCreatedAt(generation.createdAt)}</ItemDescription>
                  </ItemContent>
