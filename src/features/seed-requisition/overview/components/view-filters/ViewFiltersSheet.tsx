...
import { AdvancedTab } from '@/features/seed-requisition/overview/components/view-filters/AdvancedTab';
import { ColumnsTab } from '@/features/seed-requisition/overview/components/view-filters/ColumnsTab';
import { FiltersTab } from '@/features/seed-requisition/overview/components/view-filters/FiltersTab';
import { GroupingTab } from '@/features/seed-requisition/overview/components/view-filters/GroupingTab';
import {
...
function statusFromFilters(columnFilters: ColumnFiltersState): SeedRequisitionStatus | undefined {
  const values = columnFilters.find((filter) => filter.id === 'status')?.value;
  if (!Array.isArray(values) || values.length !== 1) return undefined;
  const [status] = values;
  return typeof status === 'string' && isSeedRequisitionStatus(status) ? status : undefined;
}

function liveManualSearch(table: RequisitionsTable) {
...
export function ViewFiltersSheet({
  ...
  onAppliedStatus: (status: SeedRequisitionStatus | undefined) => void;
}) {
