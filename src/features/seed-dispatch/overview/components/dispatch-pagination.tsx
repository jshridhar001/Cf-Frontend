...
import {
  isSeedDispatchPageSize,
  SEED_DISPATCH_PAGE_SIZES,
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(value) => {
              ...
              if (!isSeedDispatchPageSize(nextSize) || nextSize === pageSize) return;
              table.setPageSize(nextSize);
            }}
          >
            <SelectTrigger
              aria-label="Dispatches per page"
              className="h-11 w-full min-w-0 sm:h-9 sm:w-28"
            >
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {SEED_DISPATCH_PAGE_SIZES.map((size) => (
