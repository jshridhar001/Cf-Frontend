...
function searchHaystack(dispatch: SeedDispatch): string {
...
export const advancedGlobalFilterFn = constructFilterFn({
  filter: (_dataValue, filterValue, row) => {
    ...
    const dispatch = row.original as SeedDispatch;
    const query = filterValue.manualSearch?.trim().toLowerCase() ?? '';
    if (query && !searchHaystack(dispatch).includes(query)) return false;
